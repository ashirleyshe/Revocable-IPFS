...
(async () => {
    ...
    while (tmp--) {
        ...
        const procChildgen = new Promise<any>((resolve, reject) => {
            const yourscript = execgen('ipfs pin ls -q > ../log/pinls.txt', async (error, stdout, stderr) => {
                // console.log('genfile');
                if (error !== null) {
                    console.log(`exec error: ${error}`);
                    reject(error);
        ...
        var text = require('fs').readFileSync('../log/pinls.txt', 'utf8')
